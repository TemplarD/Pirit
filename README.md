...
- **Логирование**: все события безопасности

### Временный доступ для демонстрации:
```bash
# Дать доступ на 1 час
curl -X POST http://localhost:3004/api/admin/access \
...
- **Настройки сайта**: категории, пользователи, SEO

### Доступ:
- **Локально**: http://localhost:3001
- **Production**: по белому IP + JWT токены
