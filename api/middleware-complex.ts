...
import { isAllowedIP, logSecurityEvent, checkRateLimit } from '@/lib/db'
...
export async function middleware(request: NextRequest) {
  ...
  const rateLimit = await checkRateLimit(ip, 100, 3600000) // 100 запросов в час
  if (!rateLimit.allowed) {
    logSecurityEvent('RATE_LIMIT_EXCEEDED', { ip, url: request.url })
